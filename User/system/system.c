...
void SystemClock_Config(void) {
  ...
  RCC_OscInitStruct.PLL.PLLM =
      5; // 主PLL和音频PLL分频系数(PLL之前的分频),取值范围:1~63.
  ...
  RCC_OscInitStruct.PLL.PLLFRACN =
      0; // 指定PLL1 VCO的乘数的小数部分。它应该是0到8191之间的值
  ...
  RCC_ClkInitStruct.SYSCLKSource =
      RCC_SYSCLKSOURCE_PLLCLK; // 作为系统时钟的时钟源
  RCC_ClkInitStruct.SYSCLKDivider = RCC_SYSCLK_DIV1; // 系统时钟分频器
  RCC_ClkInitStruct.AHBCLKDivider =
      RCC_HCLK_DIV2; // AHB时钟（HCLK）分频器，该时钟来自系统时钟（SYSCLK）
  RCC_ClkInitStruct.APB3CLKDivider =
      RCC_APB3_DIV2; // APB3时钟（D1PCLK1）分频器，该时钟源自AHB时钟（HCLK）
  RCC_ClkInitStruct.APB1CLKDivider =
      RCC_APB1_DIV2; // APB1时钟（PCLK1）分频器，该时钟源自AHB时钟（HCLK）
  RCC_ClkInitStruct.APB2CLKDivider =
      RCC_APB2_DIV2; // APB2时钟（PCLK2）分频器，该时钟源自AHB时钟（HCLK）
  RCC_ClkInitStruct.APB4CLKDivider =
      RCC_APB4_DIV2; // APB4时钟（D3PCLK1）分频器，该时钟源自AHB时钟（HCLK）
...
//-----------------------------------------------------------------
// void MPU_Set_Protection(u32 Number,u32 BASEADDR,u32 SIZE,u8 AP,u8 DE,u8 S,u8
// C,u8 B)
//-----------------------------------------------------------------
//
// 函数功能: 配置MPU
...
//
//-----------------------------------------------------------------
void MPU_Set_Protection(u32 Number, u32 BASEADDR, u32 SIZE, u8 AP, u8 DE, u8 S,
                        u8 C, u8 B) {
...
void MPU_Memory_Protection(void) {
  // 配置AXI
  // SRAM，共512KB字节，全访问，允许指令访问，禁止共享，允许缓存，允许缓冲
  MPU_Set_Protection(MPU_REGION_NUMBER0, 0x24000000, MPU_REGION_SIZE_512KB,
                     MPU_REGION_FULL_ACCESS, MPU_INSTRUCTION_ACCESS_ENABLE,
                     MPU_ACCESS_NOT_SHAREABLE, MPU_ACCESS_CACHEABLE,
                     MPU_ACCESS_BUFFERABLE);

  // 配置FMC，共64MB字节，全访问，允许指令访问，禁止共享，禁止缓存，允许缓冲
  MPU_Set_Protection(MPU_REGION_NUMBER1, 0x60000000, MPU_REGION_SIZE_64MB,
                     MPU_REGION_FULL_ACCESS, MPU_INSTRUCTION_ACCESS_ENABLE,
