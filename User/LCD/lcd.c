...
void LCD_SetCursor(u16 Xpos, u16 Ypos) {
  ...
  } else if (lcddev.id == SPFD5408_ID) // 寄存器每次写入 16bit
  {
    ...
    LCD_WriteReg(lcddev.setycmd, Ypos);
             ...
             NT35510_ID) // 寄存器每次写入 8bit, 16bit 数据需分两次写入
  {
...
//
//					 LCD_D0  -> PD14
//LCD_RS  -> PD11 					 LCD_D1  -> PD15				LCD_RST -> PH4
//					 LCD_D2  -> PD0
//LCD_LED -> PB8 					 LCD_D3  -> PD1					LCD_NOE -> PD4
//					 LCD_D4  -> PE7
//LCD_NWE -> PD5 					 LCD_D5  -> PE8					LCD_CS  -> PD7
//					 LCD_D6  -> PE9
//					 LCD_D7  -> PE10
//					 LCD_D8  -> PE11
//					 LCD_D9  -> PE12
...
void LCD_Init(void) {
  ...
  SRAM_Handler.Init.DataAddressMux =
      FMC_DATA_ADDRESS_MUX_DISABLE;                    // 不复用数据线
  SRAM_Handler.Init.MemoryType = FMC_MEMORY_TYPE_SRAM; // SRAM
  SRAM_Handler.Init.MemoryDataWidth =
      FMC_NORSRAM_MEM_BUS_WIDTH_16; // 16位数据宽度
  SRAM_Handler.Init.BurstAccessMode =
      FMC_BURST_ACCESS_MODE_DISABLE; // 是否使能突发访问,仅对同步突发存储器有效,此处未用到
  SRAM_Handler.Init.WaitSignalPolarity =
  ...
  SRAM_Handler.Init.WaitSignal =
      FMC_WAIT_SIGNAL_DISABLE; // 等待使能位,此处未用到
  SRAM_Handler.Init.ExtendedMode =
      FMC_EXTENDED_MODE_ENABLE; // 读写使用不同的时序
  SRAM_Handler.Init.AsynchronousWait =
  ...
  FSMC_ReadWriteTim.AddressSetupTime =
      0x10; // 地址建立时间(ADDSET)为16个HCLK 1/200M*16=5ns*16=80ns
  FSMC_ReadWriteTim.AddressHoldTime = 0x01;
  FSMC_ReadWriteTim.DataSetupTime =
      0x4D; // 数据保存时间(DATAST)为77个HCLK	=5ns*77=385ns
  ...
  } else if (lcddev.id == NT35510_ID)                  // NT35510
  {
    ...
    LCD_WriteReg(0x3600,
                 0x60); // bit3  Color selector switch control“0”= RGB,“1” = BGR
